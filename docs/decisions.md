...
### Поиск ограничен простым substring search по ФИО
- `search` применяется к `student_profiles.full_name`.
- Поиск реализован через case-insensitive `ILIKE '%query%'`.
- Причина: этого достаточно для foundation-уровня и соответствует требованиям текущего шага без premature filtering system.
...
### Filter button оставлена как UI-only элемент
- Кнопка фильтра отображается рядом со строкой поиска.
- Полная фильтрация не реализовывалась.
- Причина: это следует требованиям текущего шага и не добавляет лишнюю бизнес-логику.
