...
def resolve_golden_template(
    *,
    ...
    if genre == "educational" and product_mode == "structured_explanation":
        return {
            ...
            "rules": [
                ...
                "Не смешивай markdown heading и bold в одной строке.",
            ],
        }
    ...
    if genre == "fiction" and product_mode == "basic_simplify":
        return {
            "template_id": "fiction_basic_simplify",
            "title": "Художественная адаптация с сохранением сцены",
            "structure": [
            ...
            "rules": [
                "Улучшай читаемость без пересказа сюжета.",
                "Сохраняй атмосферу, образность, эмоциональную динамику и relationships персонажей.",
                "Не превращай fiction в informational retelling.",
            ],
        }

    if genre == "fiction" and product_mode == "structured_explanation":
        return {
            "template_id": "fiction_structured_explanation",
            "title": "Художественная адаптация по эпизодам",
            "structure": [
                "# [Название сцены]",
                "### Эпизод 1",
                "### Эпизод 2",
            ...
            "rules": [
                "Эпизоды должны сохранять narrative order.",
                "Не разрывай эмоциональную сцепку между эпизодами.",
                "Не подменяй художественную ткань сухим объяснением.",
            ],
        }

    if genre == "fiction" and product_mode == "key_points_focus":
        return {
            "template_id": "fiction_key_points_focus",
            "title": "Художественные опорные точки",
            "structure": [
                "# Главное в сцене",
                "- ключевое действие",
            ...
            "rules": [
                "Сокращай осторожно и не разрушай художественное впечатление.",
                "Не превращай текст в сухой summary.",
                "Сохраняй names, roles, imagery and tone.",
            ],
        }

    if genre == "scientific_popular" and product_mode == "basic_simplify":
        return {
            "template_id": "scientific_popular_basic_simplify",
            "title": "Научно-популярное объяснение с ясной иерархией",
            "structure": [
                "# [Тема]",
            ...
            "rules": [
                "Сохраняй scientific correctness и causal relationships.",
                "Снижая cognitive load, не удаляй key concepts.",
                "Поясняй терминологию без добавления новых фактов.",
            ],
        }

    if genre == "scientific_popular" and product_mode == "structured_explanation":
        return {
            "template_id": "scientific_popular_structured_explanation",
            "title": "Научно-популярное пошаговое объяснение",
            "structure": [
                "# [Краткий заголовок по теме]",
                "## Короткое объяснение",
                "### Шаг 1. Исходное условие",
                "### Шаг 2. Основной механизм",
                "### Шаг 3. Что получается в результате",
                "## Почему это важно",
            ],
            "rules": [
                "Шаги должны соответствовать реальной научной или объяснительной логике текста.",
                "Используй 3-5 крупных смысловых блоков, а не множество микрошагов.",
                "Используй только один формат заголовков шагов: «### Шаг 1. Название шага».",
                "Не используй вложенные заголовки вида «## ### Шаг» и не оборачивай markdown-заголовки в жирный шрифт.",
                "Не подменяй причину и следствие.",
                "Не убирай промежуточные звенья, если без них ломается понимание.",
            ],
        }

    if genre == "scientific_popular" and product_mode == "key_points_focus":
        return {
            "template_id": "scientific_popular_key_points_focus",
            "title": "Научно-популярные опорные идеи",
            "structure": [
                "# Главное",
                "- ключевое понятие",
                "- ключевая связь",
                ...
                "- важный термин",
                "## Запомнить",
                "- главный вывод без расширения source scope",
            ],
            "rules": [
                "Сохраняй educational value и scientific correctness.",
                "Не превращай ключевые идеи в бытовые обобщения.",
                "Не удаляй важные понятия и causal links.",
            ],
        }
    ...
    if genre == "legal" and (strategy_mode == "mode_b" or product_mode == "key_points_focus"):
        return {
            ...
            "rules": [
                ...
                "Не меняй направление действия и агента: субъект действия должен оставаться тем же, что и в исходнике.",
    ...
    if product_mode == "structured_explanation":
        ...
        if genre == "fiction":
            ...
            if genre == "scientific_popular":
                required_structure.extend(
                    [
                        "Краткий заголовок темы вида «# ...».",
                        "Короткий вводный блок вида «## Короткое объяснение».",
                        "Итоговый блок «## Почему это важно».",
                    ]
                )
                format_instructions.extend(
                    [
                        "Для scientific_popular используй только формат шагов «### Шаг 1. Название шага».",
                        "Не создавай вложенные заголовки вида «## ### Шаг» или «# ### Шаг».",
                        "Не оборачивай markdown-заголовки шагов в жирный шрифт.",
                    ]
                )
                validation_hints.extend(
                    [
                        "Для scientific_popular шаги должны быть оформлены только как «### Шаг 1. ...».",
                        "Для scientific_popular ожидаются вводный блок «## Короткое объяснение» и итоговый блок «## Почему это важно».",
                    ]
                )
